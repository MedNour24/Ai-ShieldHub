...
class UserModel {
    ...
    /**
     * Enregistrer une modification d'email avec IP
     */
    public function logEmailChange($userId, $oldEmail, $newEmail, $ipAddress) {
        try {
            $log = [
                'user_id' => $userId,
                'old_email' => $oldEmail,
            ...
            $sql = "SELECT email_change_history FROM users WHERE id = ?";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$userId]);
            $user = $stmt->fetch(PDO::FETCH_ASSOC);
            
            $history = [];
            if (!empty($user['email_change_history'])) {
                $history = json_decode($user['email_change_history'], true) ?: [];
            }
            ...
            // Mettre à jour
            $sql = "UPDATE users SET email_change_history = ? WHERE id = ?";
            $stmt = $this->db->prepare($sql);
            return $stmt->execute([json_encode($history), $userId]);
            
        } catch(PDOException $e) {
            error_log("Erreur log changement email: " . $e->getMessage());
            return false;
        }
    }

    /**
     * Créer une notification avec informations détaillées
     */
    public function createDetailedNotification($type, $title, $message, $userId = null, $severity = 'low', $metadata = []) {
        try {
            $notification = [
            ...
            error_log("Erreur création notification détaillée: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function getAllAdmins() {
        try {
            ...
            error_log("Erreur récupération administrateurs: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function verifyPasswordResetCode($email, $code) {
        try {
            ...
            $tableExists = $this->checkTableExists('password_resets');
            if (!$tableExists) {
                return [
                    'valid' => false,
