 ...
 * FIXED VERSION - CSRF Token Validation Removed
 */
...
class RequestHandler
{
    ...
    private function validateRequestData(string $action): void
    {
        ...
        switch ($action) {
            ...
            case 'listByTournoi':
                // Validate tournament ID
                if (empty($_GET['id_tournoi']) || !filter_var($_GET['id_tournoi'], FILTER_VALIDATE_INT)) {
                    throw new Exception("ID tournoi invalide ou manquant");
                }
                $isValid = true;
                break;
