...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Team Management System Initializing...');
    addValidationStyles();
    
    // Load data in sequence
    loadTournois();
    
    // Wait a bit before loading teams to ensure tournaments are loaded
    setTimeout(() => {
        loadTeams();
        loadStatistics();
    }, 500);
    ...
    fetch(`${API_ENDPOINT}?action=getTournois`)
        ...
        .then(data => {
            if (data.success && data.data) {
                ...
                if (data.data.tournois) {
                    allTournois = data.data.tournois;
                } else if (Array.isArray(data.data)) {
                    allTournois = data.data;
                } else {
                    allTournois = [];
                }
                populateTournoiFilters();
            } else {
                console.warn('No tournaments found:', data);
                allTournois = [];
                populateTournoiFilters();
            }
        })
...
function populateTournoiFilters() {
    ...
    // Clear existing options except the first one (All tournaments / Select tournament)
    if (filter) {
    ...
    if (editSelect) {
        const firstOption = editSelect.options[0];
        editSelect.innerHTML = '';
        editSelect.appendChild(firstOption);
    }
    
    if (!allTournois || allTournois.length === 0) {
        console.log('No tournaments to populate');
        return;
    }
    
    console.log('Populating tournaments:', allTournois);
